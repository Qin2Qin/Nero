...
- Optional AI draft polishing is off by default. When enabled, Nero sends only the current draft and minimal proposal context to the official OpenRouter HTTPS chat-completions endpoint; OAuth tokens, Xero tenant IDs, and raw Xero payloads are never included in the prompt.
